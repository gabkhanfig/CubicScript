...
//     var frame = StackFrame.pushFrame(&threadLocalStack, 256, undefined, &captureReturn.value) catch {
...

            {
                const instructionPtrIncrement = blk: {
                    if (operands.argCount == 0) {
                        ...
                        const out = (operands.argCount / @sizeOf(Bytecode.FunctionArg)) + 3; // add 3 to be instruction after + immediate
                        break :blk out;
                    }
                };
                // Forcefully increment the instruction pointer here, rather than later, to avoid
