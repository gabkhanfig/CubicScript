...
typedef struct CubsStructContext CubsStructContext;
...
typedef struct {
    ...
    void* returnValueDst;
    const CubsStructContext** returnContextDst;
...
/// `offset` is an offset from the start of the current stack frame (excluding reserved slots) from as intervals of 8 bytes
const struct CubsStructContext* cubs_interpreter_stack_context_at(size_t offset);

/// If `context->sizeOfType > sizeof(size_t)`, fills the following contexts to NULL.
void cubs_interpreter_stack_set_context_at(size_t offset, const struct CubsStructContext* context);
